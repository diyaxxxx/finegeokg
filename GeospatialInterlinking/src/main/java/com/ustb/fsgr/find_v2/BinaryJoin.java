...
public class BinaryJoin {
    ...
    public List<Map<Integer, GeoNode>> find(FindMap findMap, List<GeoSpatialRelation> gsrs){
        ...
        for (int i = 0; i < findMap.getRelations().size(); i++) {
            // 获取第一个
            PriorityRelation relation  = priorityRelations.get(i);
            ...
            if (tbl1.size() == 0 && tbl2.size() == 0) {
                List<GeoSpatialRelation> rs = find(typeCode, gsrs);
            ...
            if (tbl1.size() == 0 && tbl2.size() > 0) {
                ...
                resultT = joinTables(resultT, relation.r, gsrs);
            }
    ...
    private List<GeoSpatialRelation> find(int typecode, List<GeoSpatialRelation> gsrs) {
        List<GeoSpatialRelation> result = new ArrayList<>();
        for (GeoSpatialRelation gsr : gsrs) {
            if (gsr.getGsrGeoType() == typecode){
                result.add(gsr);
            }
        }
        return result;
    ...
    private List<Map<Integer, GeoNode>> joinTables(List<Map<Integer, GeoNode>> resultT, FindMap.Relation r, List<GeoSpatialRelation> gsrs){
        ...
        for (Map<Integer, GeoNode> re : resultT) {
            ...
            for (GeoSpatialRelation gsr : gsrs) {
                if (gsr.getTo().getId().equals(to) && gsr.getFrom().getId().equals(from)) {
                    result.add(re);
                    break;
                }
            }
